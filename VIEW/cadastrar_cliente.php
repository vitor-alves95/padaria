...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    } elseif (!preg_match('/^\d{3}\.?\d{3}\.?\d{3}\-?\d{2}$/', $cpf)) { // Regex básica para CPF
        $erros_validacao[] = 'Formato de CPF inválido. Use XXX.XXX.XXX-XX ou apenas números.';
    }

    if (empty($erros_validacao)) {
        // Opcional: Remover pontos e traços do CPF antes de salvar no BD se a coluna for apenas números
        $cpf_limpo = preg_replace('/[^0-9]/', '', $cpf);

        if ($clienteDAL->inserirCliente($nome, $email, $telefone, $endereco, $cpf_limpo)) {
            $mensagem_sucesso = 'Cliente cadastrado com sucesso!';
            // Limpa os campos do formulário após o sucesso
            $nome = $email = $telefone = $endereco = $cpf = '';
        } else {
            $mensagem_erro = 'Erro ao cadastrar cliente. Tente novamente.';
        }
    } else {
...
<head>
    ...
    <title>Cadastrar Cliente - Padaria</title>
...
<body>
    ...
    <main>
        <div class="container">
            <h3 class="light">Cadastrar Novo Cliente</h3>
            <div class="row">
                    ...
                    <div class="card-panel z-depth-2">
                        <form action="cadastrar_cliente.php" method="POST">
                            ...
                            <div class="input-field">
                                ...
                                <input type="text" id="telefone" name="telefone" class="validate" required value="<?php echo htmlspecialchars($telefone); ?>" pattern="^\(?\d{2}\)?\s?\d{4,5}\-?\d{4}$" title="Formato: (XX) XXXX-XXXX ou XXXXXXXXX">
                            ...
                            <div class="input-field">
                                ...
                                <input type="text" id="cpf" name="cpf" class="validate" required value="<?php echo htmlspecialchars($cpf); ?>" pattern="\d{3}\.?\d{3}\.?\d{3}\-?\d{2}" title="Formato: XXX.XXX.XXX-XX ou apenas números">
